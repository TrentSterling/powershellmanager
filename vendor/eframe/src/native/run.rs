...
impl<T: WinitApp> WinitAppWrapper<T> {
    ...
    fn check_redraw_requests(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.windows_next_repaint_times
            .retain(|window_id, repaint_time| {
                ...
                if let Some(window) = self.winit_app.window(*window_id) {
                    // Windows does not deliver RedrawRequested for invisible
                    // windows. Leaving ControlFlow::Poll armed here therefore
                    // spins a full CPU core forever. Paint the hidden viewport
                    // directly so pending commands and repaint state drain.
                    // Backported from upstream eframe's fix for #5229/#7776.
                    if cfg!(target_os = "windows") && window.is_visible() == Some(false) {
        ...
        // Invisible Windows viewports never receive RedrawRequested, so run
        // their due frame synchronously. handle_event_result restores Wait or
        // schedules the next finite repaint instead of leaving Poll latched.
